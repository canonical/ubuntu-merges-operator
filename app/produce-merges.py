...
def produce_merge(
    ...
    base_version = Version(base_source["Version"])
    if (
        base_version >= left_source["Version"]
        or base_version >= right_source["Version"]
    ):
        cleanup(output_dir)
        return
...
def do_merge(
    ...
    for filename in tree.walk(base_dir):
        ...
        if left_stat is None and right_stat is None:
            ...
            logging.debug(
                "preserving non-file change in %s: %s",
                right_distro,
                filename,
            )
            tree.copyfile(
...
def same_file(left_stat, left_dir, right_stat, right_dir, filename):
    ...
    if stat.S_IFMT(left_stat.st_mode) != stat.S_IFMT(right_stat.st_mode):
        ...
        if left_stat.st_size != right_stat.st_size or md5sum(
            "%s/%s" % (left_dir, filename)
        ) != md5sum(
            "%s/%s" % (right_dir, filename),
        ):
...
def merge_po(left_dir, right_dir, merged_dir, filename):
    ...
    try:
        ...
        logging.exception("PO file merge failed: %s", filename)
        return True
...
def merge_pot(left_dir, right_dir, merged_dir, filename):
    ...
    try:
        ...
        logging.exception("POT file merge failed: %s", filename)
        return True
...
def conflict_file(
    ...
    if os.path.isdir(left_src) and not os.path.islink(left_src):
        os.symlink(
            "%s.%s" % (os.path.basename(dest), left_distro.upper()),
            dest,
        )
    ...
    if os.path.isdir(right_src) and not os.path.islink(right_src):
        os.symlink(
            "%s.%s" % (os.path.basename(dest), right_distro.upper()),
            dest,
        )
...
def write_report(
    ...
    with open(filename, "wt") as report:
        ...
        for _, name in files(right_source):
            print("    %s" % name, file=report)
        print(file=report)
